...
class TaskTests(TestCase):
    ...
    def test_task_list_only_my_tasks(self):
        ...
        response = self.client.get(reverse("tasks:tasks_list"), {"only_my_tasks": "on"})
    ...
    def test_author_can_delete_task(self):
        self.client.force_login(self.author)
        response = self.client.post(reverse("tasks:tasks_delete", args=[self.task.pk]), follow=True)
    ...
    def test_any_user_can_update_task(self):
        self.client.force_login(self.other)
        response = self.client.get(reverse("tasks:tasks_update", args=[self.task.pk]))
